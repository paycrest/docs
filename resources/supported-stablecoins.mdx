...
**Solana** is a **bridged network**: off-ramp deposits and on-ramp deliveries are accepted there, while the order settles on a native network. Each bridged token has one settlement token that serves both directions. `GET /tokens` lists a Solana token only while it is bridged, and does not name the settlement network; the order does, in `bridge.settlementNetwork`. See [Bridged networks](/implementation-guides/bridged-networks).
