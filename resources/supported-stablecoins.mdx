...
Off-ramp deposits are also accepted on **Solana** (USDC) through a **bridge rail**; `GET /tokens` marks such rows with `settlementNetwork`, which names the native network that row settles on. See [Bridge rails](/implementation-guides/bridge-rails).
...
<Note>
  ...
  **Solana addresses** (bridge rail) are Base58 public keys, 32–44 characters. `refundAddress` must be a Solana address when `network` is `solana`.
</Note>
