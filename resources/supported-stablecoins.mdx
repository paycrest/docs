...
Off-ramp deposits are also accepted on **Solana** (USDC) through a **bridge rail** that settles on Base; `GET /tokens` marks such rows with `settlementNetwork`. See [Bridge rails](/implementation-guides/bridge-rails).
