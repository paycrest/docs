...
OTC orders settle through a separate escrow contract, the OTC Gateway. It is deployed at one address on every supported network, so the same value can be checked against the `gateway` field of any OTC order.

| Network | Chain ID | OTC Gateway Address | Status |
|---------|----------|---------------------|--------|
| **Base Sepolia** | 84532 | TBA | 🚧 Coming Soon |
| **Base** | 8453 | TBA | 🚧 Coming Soon |

See [OTC Desk Integration](/implementation-guides/otc-desk-integration) for how the escrow is used.
