...
Legacy **[POST /v1/sender/orders](/api-reference/sender/initiate-payment-order)** uses a flatter body (`amount`, `token`, `network`, `recipient`) with the **same** uppercase institution and currency strings; prefer v2 for new work.
