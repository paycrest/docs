...
**New:** a **bridge rail** lets an order use a chain Paycrest does not settle on. **Solana** (USDC) is enabled as an off-ramp rail: orders can be funded on Solana while settling on Base. A cross-chain bridge carries the deposit to the order's receive address; assignment, payout and settlement are unchanged. Create the order exactly as before with `source.network: "solana"` and a Solana `refundAddress`.
