...
**New:** on-ramp orders can deliver to a **bridged network**. Set `destination.recipient.network: "solana"` with the user's Solana address; the order settles on the token's settlement network (Base) through the Gateway, onto an address Paycrest controls, and Paycrest then bridges the stablecoins to the user. Each bridged token has one settlement token that serves off-ramp and on-ramp alike. Paycrest no longer settles natively on Solana: a Solana source or destination is accepted only for a bridged token.

| Surface | Change |
|---------|--------|
| `POST /v2/sender/orders` | `destination.recipient.network` accepts `solana` for bridged tokens. A symbol that is not bridged there returns **400** "Provided token or payment rail is not supported" (field `Destination`). `amount` is the settlement-side amount the sender pays for; with `amountIn: "crypto"` it is slightly above the requested amount to cover the bridge. The Solana recipient must already have a token account for the mint. **503** "Bridge route unavailable" when no bridge route or quote is available; **400** when the amount is below the bridge minimum (1 unit of the stablecoin) |
...
| `GET /v2/tokens` (and `GET /v1/tokens`) | **`settlementNetwork` is removed** from token rows, with no replacement field — a change to the September off-ramp surface. Solana tokens are listed only while they are bridged. Where an order settles is on the order, in `bridge.settlementNetwork` |
| `GET /v2/rates/{network}/{from}/{amount}/{to}` | Accepts `solana`. A bridged token is quoted on its settlement network on both sides (`buy` and `sell`); a token that is not bridged gets no quote |
...
**Migration:** if you read `settlementNetwork` from the token list, read `bridge.settlementNetwork` on the order instead (for example, for explorer links).
...
| Surface | Change |
|---------|--------|
| `GET /v2/tokens` | Bridged token rows carry **`settlementNetwork`** — the network an order funded on that network settles on, and the flag that marks a bridged token. *Removed with the on-ramp release above.* |
