...
<AccordionGroup>
  ...
  <Accordion icon="danger" title="Order Creation Errors">
    ...
    <ul>
      ...
      <li><strong>Invalid Solana refund address</strong>: For the `solana` bridge rail, <code>refundAddress</code> must be a Solana Base58 public key — every refund lands there</li>
      <li><strong>"Solana is only supported for onramp orders…"</strong>: the Solana off-ramp bridge rail is switched off; offer a native network. See <a href="/implementation-guides/bridge-rails">Bridge rails</a></li>
    ...
    <p>Bridge rails only. No bridge quote could be obtained for the requested amount, or the quote could not deliver the exact settlement amount. Nothing was created — retry shortly or offer a native network.</p>

    <h4>The user's wallet will not send the deposit ("insufficient funds for gas" / "for rent")</h4>
    <p>Bridge rails only, and not a Paycrest error — the wallet is refusing before broadcast. <code>amountToTransfer</code> covers the transfer, never the rail's native gas token.</p>
    <ul>
      <li><strong>No gas in the sending account</strong>: holding the token is not enough. Wallets show a combined portfolio balance across accounts, so gas often sits in a different account than the one sending</li>
      <li><strong>Deposit addresses are fresh</strong>: on Solana the sender also pays the rent for the deposit address's token account — about <strong>0.0015 SOL</strong>, far more than the ~0.000005 SOL "transaction fee" the wallet quotes. Budget roughly <strong>0.005 SOL</strong></li>
      <li><strong>"insufficient funds for rent" when topping the account up</strong>: the <em>funding</em> account would be left below its own rent-exempt minimum (~0.00065 SOL). Send less, or use the wallet's max-send</li>
    </ul>

    <h4>Bridge deposit was refunded (order <code>expired</code>, <code>bridge.forward.status = refunded</code>)</h4>
    <ul>
      <li><strong>Deposit below <code>bridge.forward.minAmountIn</code></strong>: the bridge refunds it to <code>source.refundAddress</code> net of its fee (<code>refundReason: PARTIAL_DEPOSIT</code>). Create a new order and send exactly <code>providerAccount.amountToTransfer</code></li>
