...
<AccordionGroup>
  ...
  <Accordion icon="danger" title="Order Creation Errors">
    ...
    <ul>
      ...
      <li><strong>"Provided token or payment rail is not supported"</strong> on a bridged network: that symbol is not bridged there (no settlement token configured) — field <code>Source</code> on off-ramp, <code>Destination</code> on on-ramp. Bridged tokens are the ones <code>GET /tokens</code> lists on that network; offer a native network</li>
