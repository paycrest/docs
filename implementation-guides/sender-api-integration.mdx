...
### Large tickets go to an OTC desk

When the amount is bigger than any retail offer will take, the same create call returns `orderType: "otc"` and the order settles through an escrow with a liquidity desk on the other side. The statuses and webhooks are the same, with a few extra steps: on a buy you must call *Start payment* before you wire, and on a sell you confirm receipt yourself. It is switched on per account. See [OTC Desk Integration](/implementation-guides/otc-desk-integration) before you rely on it.
...
- [OTC Desk Integration](/implementation-guides/otc-desk-integration) — large tickets settled through an escrow with a desk
