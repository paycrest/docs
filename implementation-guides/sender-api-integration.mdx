...
When `destination.recipient.network` is a [bridged network](/implementation-guides/bridged-networks) (e.g. `solana`), the Gateway step settles on the settlement network of the token's on-ramp route and Paycrest then bridges the stablecoins to the recipient wallet; `payment_order.settled` is sent only after that delivery succeeds.
  </Tab>
</Tabs>
...
### Off-ramp via a bridged network — send `amountToTransfer` on the origin chain

When `source.network` is a bridged network, the order settles on a native network and `providerAccount` is the bridge deposit address on the origin chain with the exact origin amount to send. Send **`providerAccount.amountToTransfer`** of `providerAccount.currency` on `providerAccount.network`, include `providerAccount.memo` when present, and watch `bridge.forward.status`. Refunds land at `source.refundAddress` on the origin chain. Full details, the `bridge` object and failure modes: [Bridged networks](/implementation-guides/bridged-networks).
...
### On-ramp to a bridged network — show `bridge.recipientAmount`

When `destination.recipient.network` is a bridged network (e.g. `solana`, only for tokens with a live on-ramp route — check with `GET /v2/rates/solana/USDC/100/NGN?side=buy`; a token without one returns `400` "Provided token or payment rail is not supported"), the order settles on the on-ramp route's settlement network (`bridge.settlementNetwork`) and Paycrest then bridges the stablecoins to `destination.recipient.address`. The response carries a `bridge` object: `amount` is the settlement-side amount the sender pays for, and **`bridge.recipientAmount`** is what the user receives — show that one. With `amountIn: "crypto"` the requested amount is what arrives (within a small slippage) and `amount` is slightly higher; with `amountIn: "fiat"`, `recipientAmount` is slightly lower than `amount`. `status: "settled"` means settlement on the settlement network; `payment_order.settled` fires only once `bridge.delivery.status` is `success`, so poll `bridge.delivery` if you do not use webhooks. On Solana the recipient must already have a token account for the stablecoin's mint. Full details: [Bridged networks](/implementation-guides/bridged-networks).
...
| `payment_order.settled` | Order complete. On-ramp to a bridged network: sent once the stablecoins reach the recipient on that network |
...
| `payment_order.bridge_deposit_detected` | Off-ramp on a bridged network: the origin-chain deposit was seen and is being carried to the settlement network. **Not a status change** — `data.status` stays `initiated`; read `data.bridge.forward` |
| `payment_order.bridge_refunded` | Off-ramp on a bridged network: a refund reached `source.refundAddress` on the origin chain. **Not a status change** — fires after `payment_order.refunded` (or `expired`); read `data.bridge.reverse` |

<Note>
  You can notify the user of a successful off-ramp at `payment_order.validated` — that's when the provider has confirmed fiat delivery. For on-ramp, use `payment_order.settled` — that's when stablecoins are confirmed onchain at the recipient's address, including after the bridge delivery on a bridged network.
</Note>

<Note>
  **Bridged networks:** on off-ramp, `validUntil` on the create response is the bridge's deposit deadline, and the order stays creditable for a delivery grace period beyond it. `payment_order.refunded` means the refund reached the settlement network; the sender only has the funds back once `payment_order.bridge_refunded` fires. On on-ramp, `status: "settled"` means settlement on the settlement network, and `payment_order.settled` waits for `bridge.delivery.status` to be `success`. See [Bridged networks](/implementation-guides/bridged-networks).
</Note>
...
async function getOrderStatus(orderId) {
  ...
  switch (data.status) {
    ...
    case 'settled':   // onramp: stablecoins delivered — or offramp fully onchain
                      // onramp to a bridged network: also wait for data.bridge?.delivery?.status === 'success'
