...
### Off-ramp via a bridge rail — send `amountToTransfer` on the origin chain

When `source.network` is a bridge rail (Solana today), the order settles on a native network and `providerAccount` is the bridge deposit address on the origin chain with the exact origin amount to send. Send **`providerAccount.amountToTransfer`** of `providerAccount.currency` on `providerAccount.network`, include `providerAccount.memo` when present, and watch `bridge.forward.status`. Refunds land at `source.refundAddress` on the origin chain. Full details, the `bridge` object and failure modes: [Bridge rails](/implementation-guides/bridge-rails).
...
| `payment_order.bridge_deposit_detected` | Bridge rails: the origin-chain deposit was seen and is being carried to the settlement network. **Not a status change** — `data.status` stays `initiated`; read `data.bridge.forward` |
| `payment_order.bridge_refunded` | Bridge rails: a refund reached `source.refundAddress` on the origin chain. **Not a status change** — fires after `payment_order.refunded` (or `expired`); read `data.bridge.reverse` |
...
<Note>
  **Bridge rails:** `validUntil` on the create response is the bridge's deposit deadline, and the order stays creditable for a delivery grace period beyond it. `payment_order.refunded` means the refund reached the settlement network; the sender only has the funds back once `payment_order.bridge_refunded` fires. See [Bridge rails](/implementation-guides/bridge-rails).
</Note>
