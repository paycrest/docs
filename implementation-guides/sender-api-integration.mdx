...
When `destination.recipient.network` is a [bridged network](/implementation-guides/bridged-networks) (e.g. `solana`), the Gateway step settles on the token's settlement network and Paycrest then bridges the stablecoins to the recipient wallet; `payment_order.settled` is sent only after that delivery succeeds.
  </Tab>
</Tabs>
