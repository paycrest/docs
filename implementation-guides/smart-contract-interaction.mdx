...
async function getExchangeRate() {
  const response = await fetch("https://api.paycrest.io/v1/rates/usdt/1/ngn?network=base");
...
async def get_exchange_rate():
    response = requests.get("https://api.paycrest.io/v1/rates/usdt/1/ngn?network=base")
    data = response.json()
...
func getExchangeRate() (float64, error) {
    resp, err := http.Get("https://api.paycrest.io/v1/rates/usdt/1/ngn?network=base")
    if err != nil {
        return 0, err
    }
...
```bash
# Get exchange rate
curl -X GET "https://api.paycrest.io/v1/rates/usdt/1/ngn?network=base"
