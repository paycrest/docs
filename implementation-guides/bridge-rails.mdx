...
<Note>
  The user also needs the rail's **native gas token** — `amountToTransfer` covers the transfer itself, nothing else. Deposit addresses are fresh, so on chains where a token account is created per holder the sender pays that rent as well as the fee: on Solana that is roughly **0.0015 SOL** for the deposit address's token account plus the signature fee, against a wallet-quoted "transaction fee" of 0.000005 SOL. A wallet with the token but no gas reports `insufficient funds for gas` or `insufficient funds for rent` and the deposit never leaves.
</Note>
