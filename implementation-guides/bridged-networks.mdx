...
Rate quotes work as on a native network: `GET /v2/rates/solana/USDC/100/NGN` returns `buy` and `sell`, both priced on the token's settlement network. A token that is not bridged gets no quote. Once an order exists, **`bridge.settlementNetwork`** on the order says where it settles, and it is the chain the order's `txHash` values are on.
...
| `400` `Source` — "Provided token or payment rail is not supported" | The symbol is not bridged on this network (no settlement token configured) | Check `GET /v2/tokens`; offer a native network |
...
A destination on a bridged network is accepted only for tokens that are bridged (listed by `GET /v2/tokens` on that network); the order settles on the token's settlement network, the same one its off-ramp orders use, and the response names it in `bridge.settlementNetwork`. On Solana, the user's wallet must already have a token account for the stablecoin's mint; Paycrest checks this at create and refuses the order otherwise.
...
| `400` `Destination` — "Provided token or payment rail is not supported" | The symbol is not bridged on this network (no settlement token configured) | Check `GET /v2/tokens`; offer a native network |
