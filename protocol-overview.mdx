...
<Steps>
  ...
  <Step title="Settlement">
    <ul>
      ...
      <li><b>On-ramp:</b> Fulfillment, validation, and settlement occur in one step: after the provision node confirms fiat receipt, the stablecoins are released onchain to the recipient. When the recipient is on a bridged network, they are released on the settlement network and Paycrest then bridges them to the recipient's address.</li>
    </ul>
  </Step>
  <Step title="Refund">
...
**Solana** (network identifier `solana`) is a **bridged network**: off-ramp deposits and on-ramp deliveries are accepted there, while the order settles on a native network and a bridge carries the funds between the two. Routes are per token and per direction, and the off-ramp and on-ramp routes for one token can settle on different networks; the rates endpoint (`side=sell` / `side=buy`) shows which directions are live, and the order's `bridge.settlementNetwork` says where it settled. See [Bridged networks](/implementation-guides/bridged-networks).
