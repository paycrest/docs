...
Off-ramp deposits are additionally accepted on **Solana** (USDC — network identifier `solana`) through a **bridge rail**: the user deposits on Solana and the order settles on a native network. `GET /tokens` names which one, per token, in `settlementNetwork`. See [Bridge rails](/implementation-guides/bridge-rails).
