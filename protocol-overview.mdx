...
**Solana** (network identifier `solana`) is a **bridged network**: off-ramp deposits and on-ramp deliveries are accepted there, while the order settles on a native network and a bridge carries the funds between the two. Each bridged token has one settlement token that serves off-ramp and on-ramp alike; `GET /tokens` lists a Solana token only while it is bridged, and the order's `bridge.settlementNetwork` says where it settles. See [Bridged networks](/implementation-guides/bridged-networks).
