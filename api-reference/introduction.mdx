...
- **`payment_order.bridge_deposit_detected`** - Bridge rails only: the origin-chain deposit was seen and is being carried to the settlement network. Not a status change — `data.status` stays `initiated`; read `data.bridge.forward`
- **`payment_order.bridge_refunded`** - Bridge rails only: a refund reached `source.refundAddress` on the origin chain. Not a status change; read `data.bridge.reverse`
