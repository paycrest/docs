...
- **`payment_order.settled`** - Settlement confirmed onchain. For an on-ramp to a bridged network, sent only once the stablecoins reach the recipient on that network (`data.bridge.delivery.status` is `success`); the order's `status` reads `settled` from settlement on the settlement network onward
...
- **`payment_order.bridge_deposit_detected`** - Off-ramp on a bridged network only: the origin-chain deposit was seen and is being carried to the settlement network. Not a status change — `data.status` stays `initiated`; read `data.bridge.forward`
- **`payment_order.bridge_refunded`** - Off-ramp on a bridged network only: a refund reached `source.refundAddress` on the origin chain. Not a status change; read `data.bridge.reverse`
