...
```bash
# Production (v2 — recommended for new integrations)
https://api.paycrest.io/v2

# Legacy (v1 — fully supported, no breaking changes planned)
...
- **`POST /provider/orders/{id}/accept`** - Accept an order
- **`POST /provider/orders/{id}/decline`** - Decline an order
- **`POST /provider/orders/{id}/fulfill`** - Mark an order as fulfilled
- **`POST /provider/orders/{id}/cancel`** - Cancel an order
- **`POST /provider/balances`** - Update provider liquidity balances
...
- **`GET /rates/{token}/{amount}/{fiat}`** - Get exchange rate for a token/fiat pair
...
- **`payment_order.settling`** - Settlement transaction broadcast on-chain
- **`payment_order.settled`** - Settlement confirmed on-chain
