...
This guide walks through creating a payment order using the Sender API, the recommended path for all new integrations. You'll have a working offramp in under 10 minutes.
...
The rate endpoint is public (no API key needed). Fetch a fresh quote immediately before creating each order.

Path: **`GET /v2/rates/{network}/{token}/{amount}/{fiat}`** — network is the **first** path segment (e.g. `base`). The JSON `data` object has **`sell`** and **`buy`** sides; for a typical offramp, use **`data.sell.rate`**.
...
console.log(res.data?.sell?.rate); // offramp: fiat per crypto
```
  </Tab>
  <Tab title="Python">
```python
...
data = res.json()["data"]
print((data.get("sell") or {}).get("rate"))
```
  </Tab>
  <Tab title="Go">
```go
...
func main() {
	resp, _ := http.Get("https://api.paycrest.io/v2/rates/base/USDT/100/NGN")
	...
	fmt.Println(out.Data.Sell.Rate)
}
```
  </Tab>
</Tabs>
