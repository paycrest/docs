...
- **Order Creation**: Accepts payment orders from senders; locks stablecoins in escrow
- **Event Emission**: Emits events that the aggregator monitors to begin routing
- **Escrow Release**: Releases escrowed funds to providers after payout is verified, or returns them to senders if the order can't be fulfilled

#### Payment Flow

<Steps>
  <Step title="Order Creation">
    Sender creates an order via the Sender API or direct contract interaction. Stablecoins are locked in escrow.
  </Step>
  <Step title="Routing">
  ...
  <Step title="Fulfillment">
    Provider executes fiat payout to the recipient via a local PSP connection.
  </Step>
  <Step title="Completion">
    After payout verification, escrowed stablecoins are released to the provider onchain. If fulfillment fails, funds are automatically returned to the sender.
  </Step>
</Steps>
...
**Provision nodes** handle the execution of fiat payments to recipients. They are run by independent liquidity providers who connect their own PSP integrations.
...
- **Settlement Reporting**: Reports successful payouts back to the aggregator
