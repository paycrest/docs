...
<Note>
  **Sender Fee Model:** Sender businesses can optionally charge fees to their users. Configure a fixed fee via the dashboard, or pass `senderFeePercent` on each order request (overrides the dashboard setting). The fee is settled atomically on-chain upon successful order settlement, with no off-chain billing required.
</Note>
