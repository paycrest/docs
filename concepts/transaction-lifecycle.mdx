...
stateDiagram-v2
    [*] --> initiated
    initiated --> deposited: deposit received (direct, or carried from a bridged network)
    initiated --> expired: no deposit before validUntil
...
stateDiagram-v2
    ...
    settling --> settled: stablecoins settled onchain
    pending --> refunding: fulfillment failed
    fulfilling --> refunding: fulfillment failed
...
<Steps>
  ...
  <Step title="settled">
    ...
    On a [bridged network](/implementation-guides/bridged-networks) (e.g. `destination.recipient.network = "solana"`), `settled` means the stablecoins settled on the **settlement network**; Paycrest then bridges them to the recipient's address, tracked in `bridge.delivery`. The `payment_order.settled` webhook is sent only once `bridge.delivery.status` is `success`.
  </Step>
</Steps>
...
| `payment_order.settled` | Settlement complete. On-ramp to a bridged network: sent only once the stablecoins reach the recipient on that network (`bridge.delivery.status` is `success`) |
| `payment_order.refunding` | Refund initiated |
| `payment_order.refunded` | Refund complete |
| `payment_order.expired` | Order expired without a deposit (after `validUntil`; off-ramp on a bridged network adds a delivery grace) |
...
| `payment_order.bridge_deposit_detected` | Off-ramp on a bridged network: origin-chain deposit seen, delivery to the settlement network in flight (**not a status change** — see below) |
| `payment_order.bridge_refunded` | Off-ramp on a bridged network: refund reached the sender's origin-chain refund address (**not a status change** — see below) |
...
An order whose crypto side is on a **bridged network** (`source.network` or `destination.recipient.network` = `"solana"`) settles on a native **settlement network**, and a cross-chain bridge carries funds between the two. The order's `status` follows the lifecycle above for its direction unchanged; bridge progress is reported beside it in a `bridge` object, one leg per hop:

| Leg | Direction | Hop |
|-----|-----------|-----|
| `forward` | Off-ramp | The user's deposit on the bridged network → the order's receive address on the settlement network. The order stays `initiated` until it is credited. |
| `reverse` | Off-ramp | A refund on the settlement network → `source.refundAddress` on the bridged network |
| `delivery` | On-ramp | The settled stablecoins on the settlement network → `destination.recipient.address` on the bridged network. Starts after the order is `settled`. |

On-ramp adds no webhook event: `payment_order.settled` is held back until `bridge.delivery.status` is `success`, so it still means the recipient has the funds. When polling, `status: "settled"` alone only means settlement on the settlement network — check `bridge.delivery.status` too. A settled order is never refunded in fiat; a delivery the bridge returns is retried automatically.

For off-ramp, two events describe the hops and, like `compliance_hold`, are **not status changes**:

- `payment_order.bridge_deposit_detected` — the origin deposit was seen; `data.status` is still `initiated` and `data.bridge.forward.status` is `known_deposit_tx` or `processing`.
- `payment_order.bridge_refunded` — a refund landed at `source.refundAddress` on the origin chain; `data.bridge.reverse.status` is `success`. It follows `payment_order.refunded` (or `expired`), which on a bridged network only means the refund reached the settlement network.

Two off-ramp timing differences from a native network: `validUntil` is the bridge's deposit window rather than a Paycrest receive-address timeout, and the order stays creditable for a delivery grace period beyond it, so an on-time deposit is never expired while the bridge is still delivering. Refunds are net of bridge fees and always go to the origin-chain `refundAddress`. Field-by-field detail: [Bridged networks](/implementation-guides/bridged-networks).
