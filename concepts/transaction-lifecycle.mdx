...
```
initiated → deposited → pending → fulfilling → validated → settling → settled
...
```
initiated → pending → fulfilling → validated → settling → settled
     ↘ expired                              ↘ refunding → refunded
```
...
Monitor `payment_order.refunded` and `payment_order.expired` webhook events and implement retry logic in your integration. You can also poll [GET /sender/orders/:id](/api-reference/sender/get-order-by-id) directly.
