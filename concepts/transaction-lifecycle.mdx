...
<Note>
  OTC orders (`orderType: "otc"`) use the same statuses. They also carry an `otc.substate` that says where the escrow is, and send extra `otc.*` webhooks at each stage. See [OTC Desk Integration](/implementation-guides/otc-desk-integration#the-stages-of-an-otc-order).
</Note>
