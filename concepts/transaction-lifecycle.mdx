...
stateDiagram-v2
    [*] --> initiated
    initiated --> deposited: deposit received (direct, or delivered by a bridge rail)
...
| `payment_order.expired` | Order expired without a deposit (after `validUntil`; bridge rails add a delivery grace) |
...
| `payment_order.bridge_deposit_detected` | Bridge rails: origin-chain deposit seen, delivery to the settlement network in flight (**not a status change** — see below) |
| `payment_order.bridge_refunded` | Bridge rails: refund reached the sender's origin-chain refund address (**not a status change** — see below) |
...
An off-ramp order created on a **bridge rail** (`source.network = "solana"`) settles on a native network; a cross-chain bridge carries the user's origin-chain deposit to the order's receive address. The order's `status` follows the off-ramp lifecycle above unchanged — it stays `initiated` until the bridged delivery is credited — and bridge progress is reported beside it in a `bridge` object with a `forward` leg (deposit hop) and, once a refund is being carried back, a `reverse` leg.

Two events describe the hops and, like `compliance_hold`, are **not status changes**:
...
- `payment_order.bridge_refunded` — a refund landed at `source.refundAddress` on the origin chain; `data.bridge.reverse.status` is `success`. It follows `payment_order.refunded` (or `expired`), which on a bridge rail only means the refund reached the settlement network.

Two timing differences from a native off-ramp: `validUntil` is the bridge's deposit window rather than a Paycrest receive-address timeout, and the order stays creditable for a delivery grace period beyond it, so an on-time deposit is never expired while the bridge is still delivering. Refunds are net of bridge fees and always go to the origin-chain `refundAddress`. Field-by-field detail: [Bridge rails](/implementation-guides/bridge-rails).
